    ...
    <Link href={ `/movies/${id}` } asChild>
        <TouchableOpacity className = "w-[30%]">
            ...
            <View className='flex-row items-center justify-start gap-x-1'>
                ...
                <Text className = 'text-xs text-white font-bold uppercase'> {Math.round(vote_average / 2)}/5</Text>

            </View>
 
            <View className = ' flex-row items-center justify-between'>
                <Text className = 'text-xs text-light-300 font-medium mt-1'>{release_date?.split('-')[0]}</Text>
                {/* <Text className = 'text-xs text-light-300 font-medium mr-1 uppercase'>Movie</Text> */}
